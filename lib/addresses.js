  ...
  self.node.services.iopd.importAddress(address, function(err, suc){
	 ...
	 self.node.services.iopd.getListTransactionsByAddress(address, function(err, trans){
		 ...
		 function detail(tx, cb){
			 ...
			 if (tx.address != address || (tx.amount < 0)) {
				 totalSent +=  (tx.amount < 0) ? (tx.amount * (-1)) : tx.amount;
			 }else{
		 ...
		 async.map(trans.reverse(), detail, function(err_as, arr_as) {
			   ...
			   var transformed = {
				  ...
				  balanceSat: (totalReceived - totalSent) * 1e8,
                ...
                console.log("balance " + (totalReceived - totalSent));
