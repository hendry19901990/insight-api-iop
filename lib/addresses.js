	    ...
	    self.node.services.iopd.getAddressSummaryDB(address, function(result){
		    ...
		    if (result){
				 ...
				 self.node.services.iopd.getListTransactionsByAddress(address, function(err, trans){
						...
						var transformed = {
						  addrStr: address,
						  balance: (result.balance > 0) ? result.balance / 1e8 : 0,
						  balanceSat: result.balance,
						  totalReceived:  (result.received > 0) ? result.received / 1e8 : 0,
						  totalReceivedSat: result.received,
						  totalSent:  (result.sent > 0) ? result.sent / 1e8 : 0,
						  totalSentSat:  result.sent,
						  unconfirmedBalance: (unconfirmedBalance > 0) ?(unconfirmedBalance / 1e8): 0,
						  unconfirmedBalanceSat: unconfirmedBalance ,
						  unconfirmedTxApperances: list_txs_uncof.length, // misspelling - ew
						  txApperances: trans.length, // yuck
