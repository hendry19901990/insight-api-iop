...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.iopd.height - transaction.height + 1;
  }
  ...
  transformed.size = transaction.hex.length;  // in bytes
  if (!transaction.coinbase) {
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = iopcore.Address(output.address); //TODO return type from iopcore-node
