...
function getLastBlocks(limit, arr, first, CB){
	...
	if (first){
		client.getbestblockhash(function(err, response) {
			if (err)
				return CB(err, null);
			...
			client.getBlock(response.result, function(err_, resp) {
				 if (err_)
				    return CB(err_, null);
				
	             arr.push(resp.result.previousblockhash);
		...
		var size = arr.length;
		client.getBlock(arr[size-1], function(err_, resp) {
			if (err_)
				return CB(err_, null);
