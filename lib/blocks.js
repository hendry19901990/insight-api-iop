...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.iopd.height - blockCached.height + 1;
    ...
    self.node.getBlock(hash, function(err, block) {
      ...
      self.node.services.iopd.getBlockHeader(hash, function(err, info) {
        if (err) {
          return self.common.handleErrors(err, res);
        }
        var blockResult = self.transformBlock(block, info);
		blockResult.reward = 0;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix iopcore to give back null instead of null hash
 ...
 * Show Total Supply
 */
BlockController.prototype.getSupply = function(req, res) {
   this.node.getSupply(function(count) {  
      res.jsonp({coins: count});
   });
};
 ...
 * get the Richlist
 */
BlockController.prototype.getRichlist = function(req, res) {
   var page = (req.query.page) ? req.query.page : 1;
   this.node.getRichList(page, function(result) {  
      res.jsonp(result);
   });
};

/**
 * Show Total Network Hashps
 */
BlockController.prototype.getNetworkHashps = function(req, res) {
   this.node.getNetworkHashps(function(err, result) {  
      var networkHashps = (err != null)? 0 : result;
      res.jsonp({networkHashps: networkHashps});
   });
};
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.iopd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.iopd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new iopcore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in iopcore-node.
      var header = iopcore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [iopcore.Transaction().fromBufferReader(br)];

      self.node.services.iopd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.iopd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  getLastBlocks( limit, [], true, function(err, hashes){
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
/*
  Get the last recent blocks limit by arg
*/
function getLastBlocks  (limit, arr, first, CB){
  
	if (first){         
		self.node.services.iopd.getBestBlockHash(function(err, response) {
			if (err)
				return CB(err, null);
			...
			self.node.services.iopd.getBlockOverview(response, function(err_, resp) {
				 if (err_)
				    return CB(err_, null);

                        
				
	             arr.push(resp.prevHash);
				 
				 if (arr.length == limit)
				   return CB(null, arr);
			   
			     getLastBlocks(limit, arr, false, CB);
	        });	 
		});	
	}else{
		
		var size = arr.length;
		self.node.services.iopd.getBlockOverview(arr[size-1], function(err_, resp) {
			if (err_)
				return CB(err_, null);
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  self.node.getBlock(prevHash, function(err, block) {
    ...
    self.node.services.iopd.getBlockHeader(prevHash, function(err, info) {
      if (err) {
