...
describe('Currency', function() {
  ...
  var iopCentralData = {
    ...
    exchange_rates: {
      iop_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_iop: 0.01388998
    }
  };

  it.skip('will make live request to iop central', function(done) {
    var currency = new CurrencyController({});
    var req = {};
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.iop_usd);
        (typeof response.data.iop_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(iopCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.iop_usd);
        response.data.iop_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
